...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  // Convert YouTube URL to watch format for fullscreen
  const convertToWatchUrl = (url: string): string => {
    if (!url) return '';
    
    // Handle YouTube Shorts - convert to regular watch URL
    const shortsMatch = url.match(/youtube\.com\/shorts\/([^?]+)/);
    if (shortsMatch) {
      return `https://www.youtube.com/watch?v=${shortsMatch[1]}`;
    }
    
    // Already a watch URL or youtu.be URL
    return url;
  };

  // Handle fullscreen
  const handleFullscreen = (videoUrl: string) => {
    const embedUrl = convertToWatchUrl(videoUrl);
  ...
  const renderSpotlightModal = () => {
    ...
    const hasVideo = selectedImage.videoUrl;
    const embedUrl = hasVideo ? convertToEmbedUrl(selectedImage.videoUrl!) : '';
    ...
    return (
      ...
      >
        ...
        >
          {/* Video or Image */}
          <div className="relative bg-black" style={{ aspectRatio: hasVideo ? '9/16' : '16/9' }}>
            {hasVideo ? (
              <iframe
                id="youtube-player"
                src={embedUrl}
            ...
            <div className="absolute top-2 right-2 flex gap-2">
              {hasVideo && (
                <button
                  onClick={() => handleFullscreen(selectedImage.videoUrl!)}
  ...
  if (!isMounted) {
    return (
      <div
        className="bg-zinc-900/20 rounded-lg animate-pulse flex items-center justify-center"
  ...
  if (!images.length) {
    return (
      <div
        className="bg-zinc-900/20 rounded-lg border-2 border-dashed border-zinc-700 flex items-center justify-center"
        style={{ width: containerSize, height: containerSize }}
      >
